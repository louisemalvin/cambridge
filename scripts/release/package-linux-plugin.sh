...
build_dir=${DIRECT_WEBCAM_BUILD_DIR:-"${repo_root}/build/direct-webcam-source-release"}
staging_dir=${DIRECT_WEBCAM_STAGING_DIR:-"${build_dir}/staging"}
platform_id="linux-x86_64"
package_name="direct-webcam-source-${version}-${platform_id}"
package_root="${artifact_dir}/${package_name}"
plugin_path="${staging_dir}/obs-plugins/direct-webcam-source/bin/64bit/direct-webcam-source.so"
...
rm -rf "${package_root}"
mkdir -p "${artifact_dir}" "${package_root}/obs-plugins/direct-webcam-source/bin/64bit"
...
cp "${plugin_path}" "${package_root}/obs-plugins/direct-webcam-source/bin/64bit/"
...
printf 'artifact=direct-webcam-source\nversion=%s\nplatform=%s\nobs_libobs=%s\n' \
