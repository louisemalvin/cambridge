...
use sender_control_protocol::{
    DescribeSenderRequest, SenderAdvertisement, SenderControlAction, StartStreamRequest,
    StartStreamResponse, StartStreamStatus, PROTOCOL_VERSION, SENDER_CONTROL_PORT,
};
...
impl DiscoveryWorker {
            ...
            for result in scan {
                ...
                self.phones
                    ...
                    .or_insert(PhoneEndpoint {
                        phone: DiscoveredPhone { sender_id: sender_id.clone(), display_name },
    ...
    fn reconcile_selection(&mut self) {
        if self.phones.is_empty() {
            ...
            let sender_id = self.phones.keys().next().cloned();
            if self.selected_sender_id != sender_id {
                self.selected_sender_id = sender_id;
                self.retry_allowed = true;
                self.last_attempt = None;
            }
        } else if self.selected_sender_id.is_none() {
    ...
    fn maybe_connect(&mut self) {
        ...
        if !self.retry_allowed
            || self.last_attempt.is_some_and(|attempt| attempt.elapsed() < RETRY_INTERVAL)
        {
            return;
        }
        ...
        match send_json_request::<_, StartStreamResponse>(
            ...
            Err(error) => self
                .set_error(format!("Could not contact {}: {error}", endpoint.phone.display_name)),
        }
    }
    ...
    fn handle_response(&mut self, phone: &DiscoveredPhone, response: StartStreamResponse) {
        ...
        self.connection = match response.status {
            ...
            StartStreamStatus::ApprovalRequired => ConnectionState::WaitingForApproval(name),
            StartStreamStatus::CameraPermissionRequired => {
                ConnectionState::CameraPermissionRequired(name)
            }
            StartStreamStatus::Busy => ConnectionState::Error(
                response
                    .message
                    .unwrap_or_else(|| format!("{name} is already streaming to another receiver")),
            ),
    ...
    Response: for<'de> Deserialize<'de>,
{
    ...
    BufReader::new(stream).take(MAX_RESPONSE_BYTES).read_line(&mut response)?;
...
impl PairingStore {
    fn load(path: PathBuf) -> Result<Self> {
        if path.exists() {
            ...
            return Ok(Self { path, receiver_id: stored.receiver_id, tokens: stored.tokens });
        }
        let store = Self { path, receiver_id: Uuid::new_v4().to_string(), tokens: HashMap::new() };
        store.persist()?;
        Ok(store)
    }
    ...
    fn persist(&self) -> Result<()> {
        ...
        let stored =
            StoredPairings { receiver_id: self.receiver_id.clone(), tokens: self.tokens.clone() };
