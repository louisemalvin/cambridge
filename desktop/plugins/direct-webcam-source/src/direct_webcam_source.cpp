...
DirectWebcamSource::DirectWebcamSource(SourceConfig config, obs_source_t *source)
...
DirectWebcamSource::~DirectWebcamSource()
{
    stop();
}

SourceConfig DirectWebcamSource::configuration() const
{
    std::lock_guard<std::mutex> lock(configuration_mutex_);
    return config_;
}

bool DirectWebcamSource::start(std::string &error)
{
    if (started_) {
        return true;
    }
...
void DirectWebcamSource::update(obs_data_t *settings)
{
...
void DirectWebcamSource::render(gs_effect_t *)
{
...
std::uint32_t DirectWebcamSource::width() const
{
    std::lock_guard<std::mutex> lock(session_mutex_);
    return active_display_width_ == 0 ? contract::kDefaultCodedWidth : active_display_width_;
}

std::uint32_t DirectWebcamSource::height() const
{
    std::lock_guard<std::mutex> lock(session_mutex_);
    return active_display_height_ == 0 ? contract::kDefaultCodedHeight : active_display_height_;
}

bool DirectWebcamSource::on_hello(const HelloMessage &hello, const std::string &peer_address, std::string &error)
{
    const SourceConfig config = configuration();
...
void DirectWebcamSource::on_control_disconnect()
{
    end_session();
...
void DirectWebcamSource::on_packet_loss(std::size_t lost)
{
...
void DirectWebcamSource::on_decoder_frame(VideoFramePtr frame)
{
    if (!frame) {
        return;
    }
...
void DirectWebcamSource::on_decoder_event(const std::string &event)
{
    report(event);
}

void DirectWebcamSource::on_renderer_hardware_fallback()
{
    mailbox_.clear();
    if (decoder_) {
...
void DirectWebcamSource::report(const std::string &event) const
{
    blog(LOG_INFO, "[direct-webcam] %s", event.c_str());
}
...
{
    auto instance = std::make_unique<DirectWebcamSource>(source_config_from_settings(settings), source);
    std::string error;
    if (!instance->start(error)) {
        blog(LOG_ERROR, "[direct-webcam] source failed to start: %s", error.c_str());
        return nullptr;
    }
...
{
    delete static_cast<DirectWebcamSource *>(data);
}

std::uint32_t source_get_width(void *data)
{
    return static_cast<DirectWebcamSource *>(data)->width();
}

std::uint32_t source_get_height(void *data)
{
    return static_cast<DirectWebcamSource *>(data)->height();
}

void source_update(void *data, obs_data_t *settings)
{
    static_cast<DirectWebcamSource *>(data)->update(settings);
}

void source_video_render(void *data, gs_effect_t *effect)
{
    static_cast<DirectWebcamSource *>(data)->render(effect);
}

void source_video_tick(void *data, float seconds)
{
    static_cast<DirectWebcamSource *>(data)->tick(seconds);
}
