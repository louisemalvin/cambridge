...
internal class RtpPacketizer(
    ...
    fun sendAccessUnit(
        ...
        require(annexB.size <= DirectStreamContract.MAXIMUM_ACCESS_UNIT_BYTES) {
        ...
        val timestamp = ((timestampUs * DirectStreamContract.RTP_CLOCK_RATE_HZ) / MICROSECONDS_PER_SECOND).toInt()
    ...
    private fun sendNal(
        ...
        val maximumPayload = DirectStreamContract.RTP_MTU_BYTES - DirectStreamContract.RTP_HEADER_BYTES
    ...
    private fun sendDatagram(
        ...
        val packet = ByteArray(DirectStreamContract.RTP_HEADER_BYTES + payload.size)
        val buffer = ByteBuffer.wrap(packet).order(ByteOrder.BIG_ENDIAN)
        buffer.put((RTP_VERSION shl RTP_VERSION_SHIFT).toByte())
        buffer.put(((if (marker) RTP_MARKER_MASK else EMPTY_VALUE) or DirectStreamContract.RTP_PAYLOAD_TYPE).toByte())
        buffer.putShort((sequence and SEQUENCE_MASK).toShort())
        buffer.putInt(timestamp)
        buffer.putInt(ssrc)
        payload.copyInto(packet, DirectStreamContract.RTP_HEADER_BYTES)
