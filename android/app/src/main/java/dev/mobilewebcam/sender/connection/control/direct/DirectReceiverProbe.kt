...
class DirectReceiverProbe : ReceiverProbe {
    override suspend fun probe(endpoint: ReceiverEndpoint): Result<ReceiverCapabilities> = try {
        val requestId = UUID.randomUUID().toString()
        DirectControlConnection.connect(
            host = endpoint.host,
            port = endpoint.controlPort,
            readTimeoutMillis = DirectStreamContract.REQUEST_TIMEOUT_MILLIS,
        ).use { connection ->
            connection.send(DirectStreamContract.probe(requestId))
            ...
            when (response.stringField("type")) {
                DirectStreamContract.MESSAGE_CAPABILITIES ->
                    Result.success(response.requireCapabilities(requestId))
                DirectStreamContract.MESSAGE_ERROR -> throw ReceiverControlException(
